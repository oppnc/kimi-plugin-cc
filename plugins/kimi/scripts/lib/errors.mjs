...
export function binaryNotFoundError() {
  return formatPluginError({
    ...
    fixes: [
      ...
      "Re-run setup ( /kimi:setup  or  kimi_setup  or  companion setup )",
    ],
  });
}
...
export function mediaNotFoundError(raw, resolved, { cwd, tried } = {}) {
  ...
  return formatPluginError({
    ...
    fixes: [
      ...
      "Do not describe pixels in prose when a path is available — forward --image/--video",
    ],
  });
}
...
export function compatNotes(versionText, pluginVersion) {
  ...
  // Prefer not to hard-block on semver shapes that change; only warn on clearly ancient markers.
